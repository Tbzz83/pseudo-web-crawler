...
pub struct UrlFrontier {
    urls_to_prioritize: Vec<Option<Url>>,
}
...
impl UrlFrontier {
    ...
    pub fn with_capacity(capacity: usize) -> UrlFrontier {
        let urls_to_prioritize: Vec<Option<Url>> = Vec::with_capacity(capacity);
        UrlFrontier { urls_to_prioritize }
    }

    pub fn push_url(&mut self, url: Url) {
        self.urls_to_prioritize.push(Some(url));
    }

    pub fn prioritize_urls(&self) -> u64 {
        let mut sum: u64 = 0;
        for opt_url in &self.urls_to_prioritize {
            let Some(url) = opt_url else {
                continue;
            };
            sum = sum.wrapping_add(url.domain_rank as u64);
        }
        sum
    }
}
